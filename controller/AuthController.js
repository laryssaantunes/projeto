...
class AuthController {
  ...
  static async login(req, res) {
    ...
    res.status(200).json({
      ...
      token, // Retornando o token para o frontend
    });
  }
  ...
  static async verificaAutenticacao(req, res, next) {
    ...
    // Verificando se o token existe e dividindo corretamente
    const token = authHeader && authHeader.split(" ")[1]; // Corrigido aqui, pegando o token após o "Bearer"
    ...
    jwt.verify(token, process.env.SECRET_KEY, (err, payload) => {
      ...
      req.usuarioId = payload.id; // Armazenando o ID do usuário no request
      next(); // Continuando para a próxima função ou rota
    });
  }
}
